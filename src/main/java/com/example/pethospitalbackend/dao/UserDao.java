...
public interface UserDao extends Mapper<User> {
  ...
  @ResultType(UserDTO.class)
  @Select(
      "SELECT user_id as userId,email,role,user_class as userClass FROM user WHERE email = #{email} and password = #{password}")
  UserDTO getUserByEmailAndPassword(
      @Param("email") String email, @Param("password") String password);

  @ResultType(UserDTO.class)
  @Select(
      "SELECT user_id as userId,email,role,user_class as userClass FROM user WHERE email = #{email}")
  ...
  int updatePasswordByUserId(
      @Param("userId") String userId, @Param("newPassword") String newPassword);
